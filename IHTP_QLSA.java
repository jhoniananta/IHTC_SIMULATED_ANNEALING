...
public class IHTP_QLSA {
  ...
  private int K = 3;
  private double[] alpha = new double[K]; // probability amplitudes
  private double theta = 0.05; // learning rate

  // Soft Constraint Weights
  private int w_age, w_skill, w_cont, w_load, w_ot, w_trans, w_delay, w_opt;

  public IHTP_QLSA(String instanceFile, String solutionFile, String outputFilePath) throws IOException {
  ...
  private void initQuantumState() {
    double init = 1.0 / Math.sqrt(K);
    for (int i = 0; i < K; i++) {
      alpha[i] = init;
    }
  }

  private int measureNeighborhood() {
    double r = rand.nextDouble();
    double cumulative = 0.0;

    for (int i = 0; i < K; i++) {
      cumulative += alpha[i] * alpha[i]; // |alpha|^2
      if (r <= cumulative)
        return i;
    }
    return K - 1; // fallback
  ...
  private void updateQuantumState(int chosen, boolean reward) {
    for (int i = 0; i < K; i++) {
      if (i == chosen) {
        if (reward) {
          alpha[i] += theta * (1.0 - alpha[i]);
        } else {
          alpha[i] -= theta * alpha[i];
        }
      } else {
        if (reward) {
          alpha[i] -= theta * alpha[i];
        } else {
          alpha[i] += theta * (1.0 - alpha[i]) / (K - 1);
        }
      }
    }
  ...
  class MoveRemovePatient extends Move {
    ...
    public int getType() {
      return 0;
    } // Considered type 0 (modification of patient state)
  ...
  private static class MoveWrapper {
    ...
    public MoveWrapper(Move m, int t) {
      this.move = m;
      this.type = t;
    }
  }

  private MoveWrapper pickRandomMove() {
    if (P == 0)
      return null;

    // Quantum Selection
    int type = measureNeighborhood();
    // 0=PtSet, 1=PtSwap, 2=Nurse
    ...
    if (type == 0) {
      ...
      if (rand.nextDouble() < 0.1 && !input.PatientMandatory(p.pIndex)) {
        // Remove optional
        move = new MoveRemovePatient(p);
      } else {
        int d = rand.nextInt(D);
        int rIdx = rand.nextInt(R);
        int tIdx = rand.nextInt(T);
        move = new MoveSetPatient(p, d, input.RoomId(rIdx), input.OperatingTheaterId(tIdx));
      }
    } else if (type == 1) {
      ...
      move = new MoveSwapPatients(currentState.patients.get(i1), currentState.patients.get(i2));
    } else {
      // Nurse Move
      ...
      move = new MoveSwapNurseRoomsSingle(n1, n2, s, room1, room2);
    }

    if (move == null)
      return null;
    return new MoveWrapper(move, type);
  }

  // --- SA Loop Implementation ---
  public void run() {
    System.out.println("Starting QL-SA Simulation...");
    ...
    while (System.currentTimeMillis() < endTime) {
      iter++;
      // 1. Pick Move
      MoveWrapper wrapper = pickRandomMove();
      if (wrapper == null)
        continue;

      Move move = wrapper.move;
      int type = wrapper.type;

      // 2. Apply and Check
      move.apply(currentState);

      // Snapshot scores before re-eval
      ...
      // Update Quantum State if FEASIBLE
      // Note: User requirement says "Do NOT update quantum state if the move was
      // infeasible due to hard constraints"
      // But we determine hard limits via evaluateFull logic.
      // If reject is TRUE strictly because of Hard Violations, we skip.
      // If reject is TRUE because of Soft degradation (SA), we still update with
      // PENALTY?
      // Requirement: "Update amplitudes based on reward or penalty... after SA
      // decision"
      // Requirement: "Inside the SA loop after accept/reject decision: boolean
      // improved = (!reject && currentState.softCost < zp_soft);
      // updateQuantumState(type, improved);"
      // And: "Do NOT update quantum state if the move was infeasible due to hard
      // constraints."

      boolean feasible = (currentState.hardViolations == 0);
      // Actually, zp_hard was previous. evaluateFull updated
      // currentState.hardViolations.
      // If currentState.hardViolations > 0, it is infeasible.

      // But wait: if it was REJECTED, we don't know if the rejection was due to hard
      // or soft unless we check why.
      // My logic sets reject=true if hard>0.
      // So if hard>0, we skip update.

      if (feasible) {
        // It was feasible.
        // improved = (!reject && delta < 0).
        // If rejected by SA (Metropolis), it is NOT improved.
        // Wait, 'improved' variable calculates strictly improvement.
        // The prompt says: "boolean improved = (!reject && currentState.softCost <
        // zp_soft);"
        // So if accepted AND better cost -> Reward.
        // Else -> Penalty.

        boolean isBetter = (!reject && currentState.softCost < zp_soft);
        updateQuantumState(type, isBetter);
      }
      ...
      T_curr *= coolAlpha; // Geometris cooling schedule nya
      ...
      if (iter % 1000 == 0 && logWriter != null) {
        try {
          // Log alpha values too for debugging
    ...
    System.out.println("QL-SA Finished. Best Soft Cost: " + bestState.softCost);
  }
  ...
  public static void runSA(String inFile, String solFile, int timeLimitMinutes, String logFile, String outFile) {
    try {
      IHTP_QLSA sa = new IHTP_QLSA(inFile, solFile, outFile);
      // Configure SA
  ...
  public static void main(String[] args) {
    try {
      ...
      System.out.println("  1. java IHTP_QLSA <input.json> <input_sol.json> <output_sol.json>");
      System.out.println("  2. java IHTP_QLSA <input.json> <max_hours> <log.csv> <output_sol.json> [input_sol.json]");
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
